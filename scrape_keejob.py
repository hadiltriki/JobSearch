...
"""
Scrape Keejob.com via its public sitemap(s) and JobPosting JSON-LD.

Keejob exposes:
- https://www.keejob.com/sitemap.xml  (sitemap index)
- https://www.keejob.com/sitemap-jobs.xml  (job URLs)

Example:
  python scrape_keejob.py --max-jobs 30 --delay 0.5 --save-raw
"""
...
OUTPUT_DIR = Path("outputs_keejob")
...
def _parse_sitemap_index(xml_bytes: bytes) -> List[str]:
    """
    Return <loc> values from a <sitemapindex>.
    """
...
def _iter_sitemap_urls(xml_bytes: bytes) -> Iterable[str]:
    """
    Yield <loc> values from a <urlset> sitemap using streaming parse.
    """
...
def _job_sitemap_url_from_index(index_locs: List[str]) -> Optional[str]:
    # Prefer sitemap-jobs.xml if present.
...
def _fallback_title(html_text: str) -> str:
    ...
    if m:
        return clean_text(_strip_tags(m.group("t")).replace("| Keejob", "").strip())
    return ""
...
def _normalize_job(url: str, html_text: str, jobposting: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not description:
        # best-effort: use a short slice of visible text
...
def scrape(*, max_jobs: int, delay_s: float, save_raw: bool) -> List[Dict[str, Any]]:
    ...
    for loc in _iter_sitemap_urls(job_xml):
        if loc.startswith(BASE_URL) and "/offres-emploi/" in loc:
            job_urls.append(loc)
...
def main():
    parser = argparse.ArgumentParser(description="Scrape Keejob via sitemap + JobPosting JSON-LD.")
